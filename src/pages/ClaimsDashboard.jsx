      ...
      try {
        if (!debouncedSearch) {
          ...
          if (!cancelled) {
            setClaims(data);
            setCurrentPage(1);
            toast.success(`Loaded ${data.length} claims`);
          }
          return;
        }
...
<div className="flex items-center justify-between gap-3 mb-2">
  ...
  {/* New Claim Button on the right */}
  <div>
    <button
      onClick={() => navigate("/claims/new")}
      className="px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold rounded-xl shadow hover:opacity-90 transition-all"
    >
      + New Claim
    </button>
  </div>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-100">
                      {paginatedClaims.map((c) => (
                        <tr key={c.id} className="cursor-pointer hover:bg-gradient-to-r hover:from-indigo-50 hover:to-purple-50 hover:shadow-lg transition-all">
                          <td className="px-6 py-4">{c.id}</td>
                          <td className="px-6 py-4">{c.patient?.replace(/^Patient\//, "") ?? "N/A"}</td>
                          <td className="px-6 py-4">{c.provider?.replace(/^Practitioner\//, "") ?? "N/A"}</td>
