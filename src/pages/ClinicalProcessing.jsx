...
//     try {
//       toast.loading("Scheduling Appointment...");
//       await submitEntity(editedData, setLatestCurl, sourceId, departmentId);
//       toast.dismiss();
//       toast.success("Appointment Scheduled Successfully");
//       setAppointmentScheduled(true); // :white_tick: show navigate button
//     } catch (err) {
//       console.error(err);
//       toast.dismiss();
  ...
  const pollStatus = async (requestId, retries = 50) => {
    ...
    try {
      const statusResponse = await fetch(
        `${SAMPLE_BFF_URL}/api/request/${requestId}`
      );
      const statusData = await statusResponse.json();
      if (!isMounted.current) return;
      if (statusData.status === "PROCESSING") {
        ...
        <div className="max-w-6xl mx-auto">
          ...
          {activeStep === 1 && data && (
            <div className="relative">
              ...
              {/* Go to Scheduling - small, floating bottom-right */}
              {appointmentScheduled && (
                <Button
  onClick={() => navigate("/scheduling")}
  className="absolute right-0 -bottom-8 bg-gradient-to-r from-indigo-500 to-purple-600 text-white px-3 py-1.5 text-sm shadow-lg hover:scale-105 transition-transform rounded-lg"
>
  Go to Scheduling
</Button>
